...
import { getGroupBenefitsTranslation } from "@/lib/translations";

const GroupBenefitsSection = () => {
  const { language } = useLanguage();
  const groupCopy = getGroupBenefitsTranslation(language);
    ...
    <section className="py-16">
      <div className="container mx-auto px-4">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {groupCopy.cards.map((card, index) => (
        ...
        <div className="mt-12 text-center">
          <p className="text-sm font-semibold text-primary">
            {groupCopy.support.title}
          </p>
          <p className="mt-2 text-sm text-muted-foreground">
            {groupCopy.support.description}
          </p>
        </div>
      </div>
    </section>
